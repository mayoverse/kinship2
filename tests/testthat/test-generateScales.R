...
dimnames(ped_df_r) <- list(NULL, c("indId", "fatherId", "motherId", "gender", "steril",
    "available", "NumOther", "AffMod"))
ped_df_r <- data.frame(ped_df_r)
ped_df <- suppressWarnings(normPed(ped_df_r, na_strings = c("None", "NA")))
ped_df$NumOther <- as.numeric(ped_df$NumOther)
values = ped_df$NumOther
aff_num <- generate_aff_inds(ped_df$NumOther, threshold = 4, sup_thres_aff = TRUE)
...
test_that("generate aff inds works", {
    ...
    expect_equal(levels(as.factor(aff_num$labels)), c("Affected > to 4", "Healthy <= to 4"))
    expect_equal(aff_fact$mods, c(NA, 0, 1, 0, 1, 1, 0, 1, 0, 0))
    expect_equal(levels(as.factor(aff_fact$labels)), c("Affected are D/E", "Healthy are A/B"))
})
...
test_that("generate fill full scale off", {
    list_num <- generate_fill(ped_df$NumOther, aff_num$affected, aff_num$labels, keep_full_scale = FALSE)
    list_fact <- generate_fill(ped_df$AffMod, aff_fact$affected, aff_fact$labels, keep_full_scale = FALSE)
...
test_that("generate fill full scale on", {
    list_num <- generate_fill(ped_df$NumOther, aff_num$affected, aff_num$labels, keep_full_scale = TRUE)
    list_fact <- generate_fill(ped_df$AffMod, aff_fact$affected, aff_fact$labels, keep_full_scale = TRUE)

    expect_equal(
        list_num$fill_scale$fill,
        c("#FFFFFF", "#9AB1C4", "#36648B", "#FF0000", "grey", "#F67700", "#EEEE00")
    )
    expect_equal(
    ...
    aff_num_notsup <- generate_aff_inds(ped_df$NumOther, threshold = 4, sup_thres_aff = FALSE)
    list_num_rev <- generate_fill(ped_df$NumOther, aff_num_notsup$affected, aff_num_notsup$labels, keep_full_scale = TRUE)
    expect_equal(
        list_num_rev$fill_scale$fill,
        c("#FF0000","#F67700", "#EEEE00", "#FFFFFF",  "grey", "#9AB1C4", "#36648B")
    )
})

test_that("generate colors works on df", {
    list_num <- generate_colors(ped_df, "NumOther", threshold = 4, sup_thres_aff = TRUE)
    expect_equal(list_num$df$NumOther_aff, aff_num$mods)
    expect_equal(list_num$scale$fill$fill, c("white", "red", "grey"))
    expect_equal(list_num$scale$fill$labels,  c("Healthy <= to 4", "Affected > to 4", NA))
...
test_that("generate colors works on pedigree object", {
    ...
    expect_equal(ped_aff$scales$fill$labels,  c("Healthy <= to 120", "Affected > to 120"))
