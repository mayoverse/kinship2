...
test_that("pedigree subscripting", {
    ...
    minnped <- with(minnbreast, pedigree(id, fatherid, motherid, sex, affected = cancer,
        famid = famid))
...
test_that("pedigree other test", {
    ped2mat <- matrix(c(1, 1, 0, 0, 1, 1, 2, 0, 0, 2, 1, 3, 1, 2, 1, 1, 4, 1, 2,
        2, 1, 5, 0, 0, 2, 1, 6, 0, 0, 1, 1, 7, 3, 5, 2, 1, 8, 6, 4, 1, 1, 9, 6, 4,
        1, 1, 10, 8, 7, 2), ncol = 5, byrow = TRUE)
    ...
    ped2 <- with(ped2df, pedigree(id, dad, mom, sex, status = vitalstatus, affected = cbind(disease,
        smoker, availstatus), relation = matrix(c(8, 9, 1), ncol = 3), ))
...
test_that("pedigreeList to dataframe", {
    data(sample.ped)
    ped <- with(sample.ped, pedigree(id, father, mother, sex, affected, famid = ped))
