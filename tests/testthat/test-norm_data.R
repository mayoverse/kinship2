df <- c(1, 3, 4, 2, FALSE, NA, "1", "None", 2, 0, 0, 1, TRUE, 1, 2, "A", 3, 8, 7,
    "man", FALSE, NA, "2", "E", 4, 6, 5, "woman", FALSE, "A", 3, "A", 5, 0, 0, "f",
    FALSE, NA, 7, "E", 6, "None", 0, "m", TRUE, NA, "NA", "D", 7, 0, "0", 1, FALSE,
    "NA", 6, "A", 8, 0, 0, 1, TRUE, "None", "3", "D", 8, 0, 0, 2, TRUE, "None", "3",
    "A", 9, 9, 1, 3, FALSE, "Ab", "5", "B")
df <- matrix(df, ncol = 8, byrow = TRUE)
dimnames(df) <- list(NULL, c("IndID", "FatherID", "MotherID", "Gender", "Sterilisation",
    "Availability", "NumOther", "AffMod"))
df <- data.frame(df)
list_get <- norm_data(df, na_strings = c("None", "0", "NA"))

df_aff_num <- generate_aff_inds(list_get$norm, "NumOther", threshold = 4, sup_thres_aff = TRUE)
df_aff_fact <- generate_aff_inds(list_get$norm, "AffMod", mods_aff = c("D", "E"))
