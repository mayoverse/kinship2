...
setMethod("kindepth", "character",
    function(obj, dadid, momid, align = FALSE) {
        id <- obj
        n <- length(id)
        ...
        ## information for both, and that A's ancestors go back 3 generations, B's
        ## for only two.  If we add +1 to the depth of B and all her ancestors,
        ## then A and B will be the same depth, and will plot on the same line.
        ## Founders who marry in are also aligned. However, if an inbred pedigree,
        ## may not be a simple fix of this sort.

        ## The algorithm is 1 First deal with founders. If a founder marries in
        ## multiple times at multiple deaths (animal pedigrees), given that subject
        ## the min(depth of spouses). These subjects cause trouble for the general
        ## algorithm below: the result would depend on the data order.  2. Find any
        ## remaining mother-father pairs that are mismatched in depth.  Deal with
        ## them one at a time.  3.  The children's depth is max(father, mother) +1.
        ...
        ## the highest marriages right.  For the bad side, just get ancestors.  5.
        ## Avoid pedigree loops!  If the agood list contains anyone in abad, then
