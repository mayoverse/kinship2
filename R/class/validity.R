...
checkColnamesSlot <- function(object, slot = NULL, colnames) {
  ...
  if (length(array_names) == 0) {
    ...
    paste0(
      paste0(colnames[checkColnames(array_names, colnames)], collapse = ", "),
...
isValid <- function(object) {
  ...
  scale_cols <- c("column", "mods_labels", "fill", "border", "density", "angle")
  ...
  errors <- c(errors, checkValues(object, "ped", "momid",
    c(object@ped$id, missid)))
  errors <- c(errors, checkValues(object, "ped", "sex",
    c(1:4)))
  errors <- c(errors, checkValues(object, "ped", "steril",
    c(0, 1, NA)))
  errors <- c(errors, checkValues(object, "ped", "status",
    c(0, 1, NA)))
  errors <- c(errors, checkValues(object, "ped", "avail",
    c(0, 1, NA)))
  ...
  errors <- c(errors, checkValues(object, "rel", "id1",
    object@ped$id))
  errors <- c(errors, checkValues(object, "rel", "id2",
    object@ped$id))
