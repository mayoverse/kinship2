...
#' \\code{\\link{autohint}}
#' @export pedigree
pedigree <- function(id, dadid, momid, sex, affected, status, relation, famid, missid) {
    n <- length(id)
    ## Code transferred from noweb to markdown vignette.  Sections from the
    ## noweb/vignettes are noted here with Doc: Error and Data Checks Doc:
    ## Errors1
    if (length(momid) != n)
    ...
    # assume either 0/1/2/4 = female/male/unknown/term, or 1/2/3/4 if only 1/2
    # assume no unknowns
    if (min(sex) == 0) {
        sex <- sex + 1
    }
    ...
    if (!missing(famid)) {
        if (any(is.na(famid)))
            stop("The family id cannot contain missing values")
    ...
    if (any(duplicated(id))) {
        ...
        msgList <- min(length(duplist), 6)
        stop(paste("Duplicate subject id:", duplist[1:msgList]))
    }
    findex <- match(dadid, id, nomatch = 0)
    if (any(sex[findex] != "male")) {
        who <- unique((id[findex])[sex[findex] != "male"])
        msgList <- seq_len(min(5, length(who)))  # Don't list a zillion
        stop(paste("Id not male, but is a father:",
            paste(who[msgList], collapse = " ")))
    }
    ...
    if (any(sex[mindex] != "female")) {
        ...
        msgList <- seq_len(min(5, length(who)))
    ...
    if (any(mindex == 0 & findex != 0) || any(mindex != 0 & findex == 0)) {
        who <- id[which((mindex == 0 & findex != 0) |
            (mindex != 0 & findex == 0))]
        msgList <- seq_len(min(5, length(who)))  # Don't list a zillion
        stop(paste("Subjects must have both a father and mother,",
            "or have neither", paste(who[msgList], collapse = " ")))
    }
    ...
    if (missing(famid)) {
        ...
        temp <- list(famid = famid, id = oldid,
            findex = findex, mindex = mindex, sex = sex)
    }
    if (!missing(affected)) {
    ...
    if (!missing(relation)) {
        ...
        if (!missing(famid)) {
            temp1 <- match(paste(as.character(famid),
                as.character(id1), sep = "/"), id, nomatch = 0)
            temp2 <- match(paste(as.character(famid),
                as.character(id2), sep = "/"), id, nomatch = 0)
        } else {
        ...
        if (!missing(famid)) {
            temp$relation <- data.frame(famid = famid,
                indx1 = temp1, indx2 = temp2, code = code)
        } else {
            temp$relation <- data.frame(indx1 = temp1,
                indx2 = temp2, code = code)
        }
    }
    ## Doc: Finish
    if (missing(famid)) {
...
"[.pedigreeList" <- function(x, ..., drop = FALSE) {
    ...
    keptRows <- (seq_along(x$findex))[keep]
    x$findex <- match(x$findex[keep], keptRows, nomatch = 0)
    x$mindex <- match(x$mindex[keep], keptRows, nomatch = 0)

    # optional components
    if (!is.null(x$status))
        x$status <- x$status[keep]
    ...
    if (!is.null(x$relation)) {
        ...
        if (any(keep)) {
            ...
            x$relation$indx1 <- match(x$relation$indx1, keptRows, nomatch = 0)
            x$relation$indx2 <- match(x$relation$indx2, keptRows, nomatch = 0)
    ...
    if (length(indx) == 1) { # only one family chosen
            class(x) <- "pedigree"
    } else {
...
"[.pedigree" <- function(x, ..., drop = FALSE) {
    ...
    if (any(is.na(i)))
        paste("Subject", ..1[which(is.na(i))][1], "not found")

    z <- list(id = x$id[i], findex = match(x$findex[i], i, nomatch = 0),
        mindex = match(x$mindex[i], i, nomatch = 0), sex = x$sex[i])
    if (!is.null(x$affected)) {
        if (is.matrix(x$affected)) {
