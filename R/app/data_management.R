...
create_legend <- function(lgd_list, nbColMany = 2, size = 1) {
    ...
    for (it1 in seq_along(lgd_list)) {
        ...
        data <- data.frame(X = rep(1, nbMod), Y = 1:nbMod,
            Aff = as.character(1:nbMod))
        if (nbMod > 5) {
            nbcol <- nbColMany
        } else {
            nbcol <- 1
        ...
        grob <- ggplot(data, aes(X, Y)) +
            ...
            scale_fill_manual(
                ...
                labels = setNames(legend_labels, 1:nbMod),
                    drop = FALSE, guide = TRUE) +
            guides(fill = guide_legend(override.aes = list(col = borderColor,
                fill = fillColor, size = size * 10), ncol = nbcol)) +
            theme(
...
get_title <- function(family_sel, subfamily_sel, family_var, mod, inf_selected,
    ...
    if (subfamily_sel == "0") {
        ...
        if (short_title) {
            ...
            title <- paste0(c("Ped", family_var, mod, "-K", kin_max,
            ...
            paste0(c("Pedigree", trim_text, "of", family_var, mod, "family N°",
