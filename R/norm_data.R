...
norm_ped <- function(
  ...
  err_cols <- c(
    ...
    "idErrFa", "idErrMo", "idErrSelf", "idErrOwParent", "idErrBothParent", "idError", "error"
  )
  ...
  ped_df <- dplyr::mutate_if(ped_df, is.character, ~replace(., . %in% na_strings, NA))
  ...
  ped_df$sex <- as.character(plyr::revalue(
    as.factor(casefold(ped_df$gender, upper = FALSE)),
    sex_equiv
  ))
  ...
  if ("steril" %in% colnames(ped_df)) {
    ...
    nb_steril_parent <- nrow(ped_df[ped_df$sexErrTer == "IsSterelisedButIsParent" &
  ...
  ped_df$sexNA[!ped_df$sex %in% c("male", "female", "terminated")] <- "SexNotRecognise"
  ped_df$sexErrMoFa[is_father & is_mother] <- "IsMotherAndFather"
  ped_df$sexErrFa[is_father & ped_df$sex != "male"] <- "IsFatherButNotMale"
  ped_df$sexErrMo[is_mother & ped_df$sex != "female"] <- "IsMotherButNotFemale"
  ...
  ped_df$dadid <- paste0(as.character(pre_famid), as.character(ped_df$fatherId))
  ped_df$momid <- paste0(as.character(pre_famid), as.character(ped_df$motherId))
  ...
  # OwnParent
  id_own_parent <- ped_df$id[ped_df$id == ped_df$dadid | ped_df$id == ped_df$momid]
  ...
  ped_df <- tidyr::unite(ped_df, "idError",
    c("idErrFa", "idErrMo", "idErrSelf", "idErrOwnParent", "idErrBothParent"),
    na.rm = TRUE, sep = "_", remove = TRUE
  )
  ...
  if (try_num) {
    ...
    col_to_num <- colnames(ped_df)[!colnames(ped_df) %in% c(cols_need, cols_to_use)]
    for (i in col_to_num) {
...
norm_rel <- function(rel_df, na_strings = c("NA", ""), missid = "0") {
  ...
  if (nrow(rel_df) > 0) {
    rel_df <- dplyr::mutate_if(rel_df, is.character, ~replace(., . %in% na_strings, NA))
    ...
    rel$code <- as.character(plyr::revalue(
      as.factor(stringr::str_remove_all(
