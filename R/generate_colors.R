...
generate_colors <- function(df, col_aff, keep_full_scale = FALSE, breaks = 3,
    colors_avail = c("green", "black"), colors_aff = c("yellow2", "red"),
    ...
    border_scale <- setNames(c(colors_avail[1], colors_avail[2]), c("Available",
        "Non Available"))
    ...
    if (!keep_full_scale) {
        ...
        df$fill <- plyr::revalue(as.character(df$affected), fill_scale)
        ...
        if (!is.numeric(df[[col_aff]])) {
            ...
            levs_aff <- as.factor(df[df$affected == 1 & !is.na(df$affected),
                col_aff])
            levs_unaff <- as.factor(df[df$affected == 0 & !is.na(df$affected),
                col_aff])
            ...
            levs_unaff <- cut(df[df$affected == 0 & !is.na(df$affected),
                col_aff],
                breaks = breaks, include.lowest = TRUE)
            ...
            if (mean_aff > mean_unaff) {
                fill_scale_aff <- setNames(fill_scale_aff, levels(levs_aff))
                fill_scale_unaff <- setNames(fill_scale_unaff,
                    levels(levs_unaff))
            } else {
                fill_scale_aff <- setNames(fill_scale_aff,
                    rev(levels(levs_aff)))
                fill_scale_unaff <- setNames(fill_scale_unaff,
