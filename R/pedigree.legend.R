...
pedigree.legend <- function(ped, labels = dimnames(ped$affected)[[2]],
    ...
    # Defaults for plotting on separate page: start at the top, always
    # counter-clockwise, black/white
    initAngle <- 90
    twopi <- 2 * pi
    col <- 1

    defaultLabels <- paste("affected-", 1:naff, sep = "")
    if (is.null(labels))
    ...
    if (!new) {
        ...
        ## Settings to add to pedigree plot y-axis is flipped, so adjust angle
        ## and rotation
        initAngle <- -1 * initAngle
        twopi <- -1 * twopi

        ## track usr xy limits. With asp=1, it re-scales to have aspect ratio
        ## 1:1
        usr.orig <- par("usr")
        ...
        pctusr <- 0.1 * abs(diff(usr.asp1[3:4]))
        ...
        if (is.character(location)) {
            ...
            stop(paste("Invalid location format:",
            "either string or numerical vetor of 2"))
        }
    ...
    t2xy <- function(t) {
        t2p <- twopi * t + initAngle * pi / 180
        list(x = radius * cos(t2p), y = radius * sin(t2p))
    }
    for (i in 1L:nx) {
        n <- max(2, floor(edges * dx[i]))
        plgn <- t2xy(seq.int(x[i], x[i + 1], length.out = n))
        plgn$x <- plgn$x + center[1]
        plgn$y <- plgn$y + center[2]

        polygon(c(plgn$x, center[1]), c(plgn$y, center[2]),
            density = density[i], angle = angle[i],
            border = border[i], col = col[i], lty = lty[i])

        plgn <- t2xy(mean(x[i + 0:1]))
        if (new) {
            ## not centered at 0,0, so added center to x,y
            plgn$x <- plgn$x + center[1]
            plgn$y <- center[2] + ifelse(new, plgn$y, -1 * plgn$y)
        }
        ...
        if (!is.na(lab) && nzchar(lab)) {
            ## put lines
            lines(
                x = c(plgn$x, plgn$x + ifelse(plgn$x < center[1],
                    -1 * llen, llen)),
                y = c(plgn$y, plgn$y + ifelse(plgn$y < center[2],
                    -1 * llen, llen)))

            ## put text just beyond line-length away from pie
            text(
                x = plgn$x + ifelse(plgn$x < center[1],
                    -1.2 * llen, 1.2 * llen),
                y = plgn$y + ifelse(plgn$y < center[2],
                    -1.2 * llen, 1.2 * llen), labels[i],
                xpd = TRUE, adj = ifelse(plgn$x < center[1], 1, 0), ...)
        }
    }
    invisible(NULL)
}
