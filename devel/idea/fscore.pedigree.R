...
fscore.pedigree <- function(ped) {
    ...
    pfun1 <- function(id, dad, mom, sex) {
        ...
        if (dad[id] == 0 && mom[id] == 0) {
            ...
            return(c(Recall(dad[id], dad, mom, sex),
                Recall(mom[id], dad, mom, sex)))
        }
    }

    # Now, find all marriages (This code is essentially the same as that in
    # align.pedigree) Mothers are row 1, fathers row 2, each column a spouse
    # pair Either having children, having a hint, or a spousal relationship
    # gets you on the list (that is, any of the things that would cause this
    # marriage to part of the plotted pedigree).  Start with a hashlist:
    # fatherid * max(id) + mother
    ...
    if (!is.null(ped$relation) && any(ped$relation[, 3] == 4)) {
        ...
        temp1 <- ifelse(ped$sex[indx] == "male",
            n * indx + ped$relation[who, 2],
    ...
    # Create the founder sets for each spouse pair It will be a list of npair
    # elements, each of which is a list containing 2 vectors
    ...
    pfun2 <- function(flist, ord) {
        # This function computes the badness score for the founders of each
        # parental pair, along with a report of which of the 4 terms is the
        # 'worst'.
        tfun <- function(x, ord) {
            t1 <- ord[x[[1]]]
            t2 <- ord[x[[2]]]
        ...
        tempMin <- apply(temp, 2, min)
        tempWhich <- apply(temp == matrix(rep(tempMin, each = 4), nrow = 4),
            2, function(x) min(c(which(x))))
        cbind(tempMin, tempWhich)
    }
