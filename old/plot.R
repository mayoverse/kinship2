...
setMethod("plot", c(x = "Pedigree", y = "missing"),
    function(x, id = x$ped$id, status = x$ped$status,
    affected = x$ped$affected, avail = x$ped$avail + 1, mark = NA, label = NA,
    tips_names = NA, fill = "grey", border = "black", ggplot_gen = FALSE,
    cex = 1, symbolsize = 1, branch = 0.6, packed = TRUE, align = c(1.5, 2),
    width = 6, psize = par("pin"), title = NULL, density = c(-1, 35, 65, 20),
    ...
    keep_par = FALSE, subregion, pconnect = 0.5, ...) {
        ...
        ## vector of length [[n]] or a matrix with [[n]] rows. If not present, the
        ## default is to plot open symbols without shading or color
        ...
        ## to -1, and plotted as a question mark (?) in the plot symbol region for
        ...
        if (is.null(affected)) {
            ...
            if (is.matrix(affected)) {
                ...
                if (ncol(affected) > length(angle) ||
                    ...
                    stop(paste("More columns in the affected matrix",
        ...
        all_var_checked <- lapply(mget(c("avail", "mark",
            "label", "fill", "border")),
            function(var) {
                if (length(var) <= 1) {
                    return(rep(var, n))
        ...
        if (ncol(affected) == 1) {
            ...
            nc <- ncol(affected)
            square <- polyfun(nc, list(x = c(-0.5, -0.5, 0.5, 0.5), y = c(-0.5, 0.5,
                0.5, -0.5), theta = -c(3, 5, 7, 9) * pi / 4))
            circle <- circfun(nc)
        ...
        drawbox <- function(x, y, sex, affected, status, avail, mark, polylist,
            ...
            for (i in seq_len(length(affected))) {
                ...
                col_text <- "black"
                text(midx, midy, labels = plabel, cex = cex/length(affected), col = col_text)
                ...
                if (ggplot_gen) {
                    ...
                    p_plot <- p_plot +
                        suppressWarnings(ggplot2::geom_text(ggplot2::aes(
                        x = midx, y = midy, label = plabel,
                        text = tips, color = col_text)))
                }
            }
            if (status == 1) {
                segments(x - 0.6 * boxw, y + 1.1 * boxh,
                    x + 0.6 * boxw, y - 0.1 * boxh)
                if (ggplot_gen) {
                    p_plot <- p_plot + ggplot2::annotate("segment",
                    x = x - 0.6 * boxw, y = y + 1.1 * boxh,
                    xend = x + 0.6 * boxw, yend = y - 0.1 * boxh)
                }
            }
        ...
        legend_labels <- c("Unaffected and unavailable", "Unaffected and available",
            "Affected and unavailable", "Affected and available")
        colors_plot <- unique(c(fill, border))

        p <- ggplot2::ggplot() +
            ggplot2::scale_fill_manual(
                values = stats::setNames(colors_plot, colors_plot),
                name = "Availability",
                labels = stats::setNames(legend_labels, 1:4), drop = FALSE) +
            ggplot2::scale_color_manual(
                values = stats::setNames(colors_plot, colors_plot), guide = FALSE)
        ...
        prog_bar <- utils::txtProgressBar(0, maxlev, char = "|",
            width = 50, style = 3)
        for (i in 1:maxlev) {
            utils::setTxtProgressBar(prog_bar, i)
            for (j in seq_len(plist$n[i])) {
                k <- plist$nid[i, j]
                p <- drawbox(plist$pos[i, j], i, sex[k], affected[k, ], status[k],
                    avail[k], mark[k], polylist, fill[k], border[k], density, angle,
                    boxw, boxh, p, id[k], df)
                text(plist$pos[i, j], i + boxh + labh * 0.7, id[k],
                    cex = cex, adj = c(0.5, 1))
                if (ggplot_gen) {
                    p <- p +
                        ggplot2::annotate("text", label = id[k],
                            x = plist$pos[i, j], y = i + boxh + labh * 0.7)
                }
                if (!is.na(label[k])) {
                    # Label added if present under the individual
                    text(plist$pos[i, j], i + boxh + labh * 0.7 * 3,
                        paste0("(", label[k], ")"), cex = cex * 0.75,
                        adj = c(0.5, 1))
                }
            }
        }
        ...
        for (i in 1:maxlev) {
            ...
            if (any(plist$spouse[i, ] > 0)) {
                ...
                segments(x0 = plist$pos[i, temp] + boxw / 2,
                ...
                if (length(temp)) {
                    ...
                    tempy <- tempy + boxh / 10
                    segments(plist$pos[i, temp] + boxw / 2,
        ...
        for (i in 2:maxlev) {
            ...
            for (fam in zed) {
                ...
                if (any(plist$twins[i, who] == 3)) {
                    ...
                    if (ggplot_gen) {
                    for (i2 in seq_len(length(yy))) {
                        p <- p + ggplot2::annotate("text", label = "?",
                ...
                # Draw line to parents.  The original rule corresponded to pconnect
                # a large number, forcing the bottom of each parent-child line to
                # be at the center of the bar uniting the children.
        ...
        ## JPS 4/27/17: unique above only applies to rows unique added to for loop
        ## iterator
        imax <- length(unique(uid[uid > 0]))
        i <- 0
        if (imax > 0) {
            message("\nDrawing arcs for same Id", appendLF = TRUE)
            prog_bar <- utils::txtProgressBar(0, imax - 1, width = 50,
                style = 3, char = "|")
        }
