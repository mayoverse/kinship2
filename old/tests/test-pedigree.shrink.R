...
test_that("Pedigree shrink works", {
    ...
    expect_equal(shrink.mnf8$idList$unavail, c(137, 138, 139, 140, 144, 145, 146,
        147, 148, 150, 151, 152, 153, 154, 155, 156, 157, 158, 159, 160, 163, 164,
...
test_that("Pedigree shrink avail test", {
    ...
    set.seed(10)
    shrink1.avail.B32 <- pedigree.shrink(ped = ped1, avail = ped1$affected[, 2],
        maxBits = 32)

    set.seed(10)
    shrink1.avail.B25 <- pedigree.shrink(ped = ped1, avail = ped1$affected[, 2],
        maxBits = 25)

    expect_equal(shrink1.avail.B32$idTrimmed, c(101, 102, 107, 108, 111, 113, 121,
        122, 123, 131, 132, 134, 139))

    expect_equal(shrink1.avail.B25$idTrimmed, c(101, 102, 107, 108, 111, 113, 121,
...
test_that("Pedigree shrink with character", {
    ...
    set.seed(100)
    shrink1.p1char.B32 <- pedigree.shrink(ped = ped1char, avail = ped1char$affected[,
        2], maxBits = 32)
    expect_equal(shrink1.p1char.B32$idTrimmed, c("A-01", "A-02", "A-07", "A-08",
        "A-11", "A-13", "A-21", "A-22", "A-23", "A-31", "A-32", "A-34", "A-39"))

    set.seed(100)
    shrink1.p1char.B25 <- pedigree.shrink(ped = ped1char, avail = ped1char$affected[,
        2], maxBits = 25)
    expect_equal(shrink1.p1char.B25$idTrimmed, c("A-01", "A-02", "A-07", "A-08",
        "A-11", "A-13", "A-21", "A-22", "A-23", "A-31", "A-32", "A-34", "A-39", "A-33",
        "A-41"))
})
...
test_that("pedigree.shrink.plot works", {
    ...
    expect_doppelganger("Shrinked ped 1", plot.pedigree.shrink(shrink2, title = "Sample Pedigree 2",
        location = c(1, 1)))
    expect_doppelganger("Shrinked ped 2", plot.pedigree.shrink(shrink2, bigped = TRUE,
        title = "Sample Pedigree 2", location = "bottomright"))
})
TRUE
