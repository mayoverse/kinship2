...
Add example and write-up for fixParents. Could do this in multiple code chunks, but the basic example is this:
  
```{r, fixped}
datped2 <- sample.ped[sample.ped$ped %in% 2,]

## make 203, who is listed as a father, a female
datped2[datped2$id %in% 203, "sex"] <- 2

## remove subject 209 who is a married-in father
datped2 <- datped2[-which(datped2$id %in% 209),]

## this gets an error for 203 not being male.
ped2 <- with(datped2, pedigree(id, father, mother, sex))
## this fixes 203 and adds 209 back in.
## remove this note:: need to load in the function from fixParents.R, which is not yet incorporated in the kinship2 package installed from CRAN
fixped2 <- with(datped2, fixParents(id, father, mother, sex))
## we see how it worked
fixped2
...
Note: Could add more details here about how to add more people to show inbreeding of founders.
