...
The first thing to be done is to check on twins. They increase the complexity, since
twins need to move together.  The `ped$rel$code` object is a factor, so first turn that into numeric.
We create 3 vectors:
...
hence `align = FALSE`. If there is a hints structure entered, we retain its non-zero entries,
...
each subject in the pedigree, not to their label `ped$ped$id`.
...
- $ped$ a pedigree or pedigreeList object. In the case of
